---
title: "F1 Analysis Final Project"
...
The data is collected from an F1 dataset on Kaggle. It contains various datasets detailing races, results, drivers, constructors, qualifying results, and many more from the years 1950-2023. These datasets have separate identifiers in each of them, so we will need to join them on those identifieres for further analysis. 

[F1 Kaggle Dataset](https://www.kaggle.com/datasets/rohanrao/formula-1-world-championship-1950-2020)
...
```{r}
racesWithCircuits <- 
  right_join(races, circuits, by = "circuitId")

racesWithCircuits <- 
  drop_na(racesWithCircuits)
#join the races and circuits dataframes by circuitId , and keep all columns in circuits dataframe

resultsWithDrivers <- 
  right_join(results, drivers, by = "driverId")

resultsWithDrivers <- 
  drop_na(resultsWithDrivers)
#join the results and drivers dataframes by driverId, and keep all colummns in drivers dataframe

racesResults <- 
  full_join(racesWithCircuits, resultsWithDrivers, by = "raceId")

racesResults <- 
  drop_na(racesResults)
...
test <- 
  racesResults %>% 
  group_by(name, driverId, year) %>% 
  arrange(driverId, circuitId)
#make sure joins worked properly by checking driver's scored points on a circuit each year
```

```{r}
averageCircuitPoints <- 
  racesResults %>%
  group_by(driverRef, name) %>% 
  summarise(totalPoints = sum(points), numRaces = n()) %>%
...
removedTracks <- 
  averageCircuitPoints %>% 
  filter(!grepl("Istanbul", name))
#this track was removed in 2011 due to low attendance, thus it is impossible for drivers to have driven on this track to be inlcuded in analysis

filteredAverageCircuitPoints <- 
  removedTracks %>% 
  filter(numRaces > 10 ) %>% 
  group_by(driverRef) %>% 
  arrange(driverRef, desc(averagePoints)) 
...
```{r}
uniqueTracks <- list()
for (i in unique(filteredAverageCircuitPoints$name)) {
  uniqueTracks <- c(uniqueTracks, i)
}
print(uniqueTracks)
#this gives us all of the tracks in the filtered dataframe so that we can easily see which tracks are included for the graph
```
...
```{r}
filteredAverageCircuitPoints %>% 
  ggplot(aes(x=averagePoints, y=name)) + 
  geom_point(aes(color=driverRef)) + 
  labs(x= "Average Circuit Points", y= "Circuit", color = "Driver", title = "Average Circuit Points for Drivers") 

```

The graph below depicts the relationship between the highest average points for a circuit, by the circuit name. This graph is separated by driver, and layered with the rounded average points.We observe that Hamilton has the highest average points on F1 circuits for most circuits. 
...
```{r}
ggplot(maxAveragePoints) +
 aes(x = name, y = driverRef, fill = averagePoints) +
 geom_tile() + 
 geom_text(label = round(maxAveragePoints$averagePoints)) +
 scale_fill_viridis_c(option = "plasma", direction = 1) +
 coord_flip() +
 theme_minimal() + 
labs(x= "Max Average Circuit Points", y= "Circuit", color = "Driver", title = "Highest Average Circuit Points for Drivers")
```
